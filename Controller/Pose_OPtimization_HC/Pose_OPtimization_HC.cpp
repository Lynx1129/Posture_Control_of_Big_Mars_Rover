// File:          Pose_Optimization_HC.cpp
// Date:          2022.4.22
// Description:   优化方法：Hill Climbing final version
// Author:        Yin Siyuan
// Modifications: 
...
//data output txt
std::ofstream mycout("HC_data2.txt");
...
std::array<double,12> hill_climbing(std::array<double,12> parameter,std::array<double,3> gravity)
{ 
  // optimization step
  double step = 0.005;
  double iteration = 0;
  ...
  while(iteration<10)
  {
    std::array<double,10> step_performance;
    ...
    {
     //if step_min < current_min, update parameter
     switch(step_index)
     {
      case 0:
        parameter_opt.at(0) +=step; parameter_opt.at(0) = boundary(parameter_opt.at(0),min14,max14);
     ...
     std::cout << "step_performance is: \t" <<  step_performance.at(2) << "\t"  <<  step_performance.at(3) << "\t" 
     <<  step_performance.at(6) << "\t" <<  step_performance.at(7) << std::endl;
    ...
    {
     // if no better result could be found, break;
     break;
    }  
...
int main(int argc, char **argv) {
  ...
  while (robot->step(timeStep) != -1) {
    ...
    std::array<double,12> opt_result = hill_climbing(opt_para,gravity_normal);
    ...
    else
    {
     ...
     else
     {
      ...
      wheelmotor3->setPosition(INFINITY);
      wheelmotor4->setPosition(INFINITY);
      wheelmotor1->setVelocity(0.4);
      wheelmotor2->setVelocity(0.4);
      wheelmotor3->setVelocity(-0.4);
      wheelmotor4->setVelocity(-0.4);
