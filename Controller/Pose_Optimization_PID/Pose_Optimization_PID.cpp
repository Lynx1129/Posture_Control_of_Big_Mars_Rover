// File:          Pose_Optimization_PID.cpp
// Date:          2022.4.22
// Description:   优化方法：PID
// Author:        Yin Siyuan
// Modifications: 
...
double P_Pitch = 1; double I_Pitch = 0; double D_Pitch = 30;
double P_Roll = 1; double I_Roll = 0; double D_Roll = 30;
double P_Pos = 0.2; double I_Pos = 0; double D_Pos = 0.01;
...
//data output txt
std::ofstream mycout("PID_data2.txt");
...
int main(int argc, char **argv) {
  ...
  while (robot->step(timeStep) != -1) {
    ...
    contact_force2 = sqrt(pow(wheelsensor2_value[0],2)+pow(wheelsensor1_value[1],2)+pow(wheelsensor2_value[2],2));
    contact_force3 = sqrt(pow(wheelsensor3_value[0],2)+pow(wheelsensor1_value[1],2)+pow(wheelsensor3_value[2],2));
    contact_force4 = sqrt(pow(wheelsensor4_value[0],2)+pow(wheelsensor1_value[1],2)+pow(wheelsensor4_value[2],2));
    ...
    else
    {
     ...
     if(isContact == 1)
     {
       ...
       std::cout << "vel are: \t" << pullmotor_vel1 <<"\t" << pullmotor_vel2 <<"\t" << pullmotor_vel3 <<"\t" << pullmotor_vel4 <<std::endl;
     ...
     else
     {
      ...
      // //调节方法1 位置控制
      double sp = 0.0015; // standard pace
      double pos1 = beta1; double pos2 = beta2; double pos3 = beta3; double pos4 = beta4;
      if(contact_flag1==0){pos1 += 3*sp; pos2 += sp; pos3 += sp; pos4 +=sp;}
      else if(contact_flag2==0){pos1 -= sp; pos2 -= 3*sp; pos3 -= sp; pos4 -= sp;}
      else if(contact_flag3==0){pos1 -= sp; pos2 -= sp; pos3 -= 3*sp; pos4 -= sp;}
      else if(contact_flag4==0){pos1 += sp; pos2 += sp; pos3 += sp; pos4 += 3*sp;}
    ...
    if(i==2400){ delete robot; return 0;}
