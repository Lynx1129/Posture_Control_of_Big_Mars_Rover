// File:          Pose_Optimization_SA1.cpp
// Date:          2022.4.14
// Description:   优化方法：Simulated annealing algorithm
// Author:        Yin Siyuan
// Modifications: 
...
const double k3 = 0.8; // pullmotor locomotion
const double k4 = 4;   // row pitch penalize
const double k5 = 0;   //theta penalize
...
while(T>0.1 && delta>eps)
{
 ...
 for(int inner_circle = 0; inner_circle<L ;inner_circle++)
 {
  ...
  {
   //否则的话以一定概率接收新解
   double w = exp(-500*(stepmin_performance_index - current_performance_index)/T);
   srand(time(0));
...
int main(int argc, char **argv) {
  ...
  while (robot->step(timeStep) != -1) {
    ...
    else
    {
     ...
     wheelmotor1->setVelocity(0.6);
     wheelmotor2->setVelocity(0.6);
     wheelmotor3->setVelocity(-0.6);
     wheelmotor4->setVelocity(-0.6);
    ...
    if(i==2000){ delete robot; return 0;}
