// File:          Pose_Optimization_SA2.cpp
  // Date:          2022.4.19
// Description:   优化方法：Simulated annealing algorithm 但是加了触地检测和对应方案
// Author:        Yin Siyuan
// Modifications: 
...
int isContact, contact_flag1,contact_flag2,contact_flag3,contact_flag4;
...
//data output txt
std::ofstream mycout("SA_data.txt");
...
int array_min(std::array<double,8> performance)
{
...
while(T>0.1 && delta>eps)
{
 ...
 for(int inner_circle = 0; inner_circle<L ;inner_circle++)
 {
  ...
  std::array<double,8> step_performance; //内循环一次的表现
  ...
  if(parameter_opt.at(0)==max14||parameter_opt.at(1)==min23){step_performance.at(0)=INFINITY;}  
  ...
  if(parameter_opt.at(0)==min14||parameter_opt.at(1)==max23){step_performance.at(1)=INFINITY;} 
  parameter_opt.at(0) = previous_beta1; parameter_opt.at(1) = previous_beta2; 
  gravity_change = gravity_init; parameter_opt.at(9) = previous_Roll;
  ...
  if(parameter_opt.at(1)==min23||parameter_opt.at(3)==max14){step_performance.at(2)=INFINITY;} 
  ...
  if(parameter_opt.at(1)==max23||parameter_opt.at(3)==min14){step_performance.at(3)=INFINITY;} 
  parameter_opt.at(1) = previous_beta2; parameter_opt.at(3) = previous_beta4; 
  gravity_change = gravity_init; parameter_opt.at(10) = previous_Pitch;
  ...
  if(parameter_opt.at(2)==min23||parameter_opt.at(3)==max14){step_performance.at(4)=INFINITY;} 
  ...
  if(parameter_opt.at(2)==max23||parameter_opt.at(3)==min14){step_performance.at(5)=INFINITY;} 
  parameter_opt.at(2) = previous_beta3; parameter_opt.at(3) = previous_beta4;
  gravity_change = gravity_init; parameter_opt.at(9) = previous_Roll;
  ...
  if(parameter_opt.at(0)==max14||parameter_opt.at(2)==min23){step_performance.at(6)=INFINITY;} 
  ...
  if(parameter_opt.at(0)==min14||parameter_opt.at(2)==max23){step_performance.at(7)=INFINITY;} 
  parameter_opt.at(0) = previous_beta1; parameter_opt.at(2) = previous_beta3;
  gravity_change = gravity_init; parameter_opt.at(10) = previous_Pitch;  
...
int main(int argc, char **argv) {
  ...
  while (robot->step(timeStep) != -1) {
    ...
    isContact = 1; contact_flag1=1; contact_flag2=1; contact_flag3=1; contact_flag4=1;
    if(abs(contact_force1 <0.5)){contact_flag1 = 0;}
    if(abs(contact_force2 <=0.5)){contact_flag2 = 0;}
    if(abs(contact_force3 <=0.5)){contact_flag3 = 0;}
    if(abs(contact_force4 <=0.5)){contact_flag4 = 0;}
    ...
    else
    {
     ...
     if(isContact == 1)
     {
      ...
      wheelmotor1->setVelocity(1.0);
      wheelmotor2->setVelocity(1.0);
      wheelmotor3->setVelocity(-1.0);
      wheelmotor4->setVelocity(-1.0);
      }
     else
     {
      // 有轮离地摆烂
      ...
      // 调节方法1 位置控制
      // wheel1离地
      double pos1_1 = beta1 + 0.01;
      double pos1_2 = beta2 + 0.005;
      double pos1_3 = beta3 + 0.005;
      double pos1_4 = beta4 + 0.005;
      pos1_1 = boundary(pos1_1, min14, max14);
      pos1_2 = boundary(pos1_2, min23, max23);
      pos1_3 = boundary(pos1_3, min23, max23);
      pos1_4 = boundary(pos1_4, min14, max14);
      if(contact_flag1==0)
      {
       pullmotor1->setPosition(pos1_1 + 4.71);
       pullmotor2->setPosition(pos1_2);
       pullmotor3->setPosition(pos1_3);
       pullmotor4->setPosition(pos1_4);
      }
      
      //wheel2离地
      double pos2_1 = beta1 - 0.005;
      double pos2_2 = beta2 - 0.01;
      double pos2_3 = beta3 - 0.005;
      double pos2_4 = beta4 - 0.005;
      pos2_1 = boundary(pos2_1, min14, max14);
      pos2_2 = boundary(pos2_2, min23, max23);
      pos2_3 = boundary(pos2_3, min23, max23);
      pos2_4 = boundary(pos2_4, min14, max14);
      if(contact_flag2==0)
      {
       pullmotor1->setPosition(pos2_1 + 4.71);
       pullmotor2->setPosition(pos2_2);
       pullmotor3->setPosition(pos2_3);
       pullmotor4->setPosition(pos2_4);
      }
      
      //wheel3离地
      double pos3_1 = beta1 - 0.005;
      double pos3_2 = beta2 - 0.005;
      double pos3_3 = beta3 - 0.01;
      double pos3_4 = beta4 - 0.005;
      pos3_1 = boundary(pos3_1, min14, max14);
      pos3_2 = boundary(pos3_2, min23, max23);
      pos3_3 = boundary(pos3_3, min23, max23);
      pos3_4 = boundary(pos3_4, min14, max14);
      if(contact_flag3==0)
      {
       pullmotor1->setPosition(pos3_1 + 4.71);
       pullmotor2->setPosition(pos3_2);
       pullmotor3->setPosition(pos3_3);
       pullmotor4->setPosition(pos3_4);
      }
      
      //wheel4离地
      double pos4_1 = beta1 + 0.005;
      double pos4_2 = beta2 + 0.005;
      double pos4_3 = beta3 + 0.005;
      double pos4_4 = beta4 + 0.01;
      pos4_1 = boundary(pos4_1, min14, max14);
      pos4_2 = boundary(pos4_2, min23, max23);
      pos4_3 = boundary(pos4_3, min23, max23);
      pos4_4 = boundary(pos4_4, min14, max14);
      if(contact_flag4==0)
      {
       pullmotor1->setPosition(pos4_1 + 4.71);
       pullmotor2->setPosition(pos4_2);
       pullmotor3->setPosition(pos4_3);
       pullmotor4->setPosition(pos4_4);
      }
      ...
      // double standard_vel = 0.01; // standard velocity
      // if(contact_flag1==0){vel1 = 2*standard_vel; vel2 =standard_vel; vel3 = standard_vel; vel4 = standard_vel; }
      // else if(contact_flag2==0){vel1 = -standard_vel; vel2 = -2*standard_vel; vel3 = -standard_vel; vel4 = -standard_vel;}
      // else if(contact_flag3==0){vel1 = -standard_vel; vel2 = -standard_vel; vel3 = -2*standard_vel; vel4 = -standard_vel;}
      // else if(contact_flag4==0){vel1 = standard_vel; vel2 = standard_vel; vel3 = standard_vel; vel4 = 2*standard_vel;}
      ...
      wheelmotor3->setPosition(INFINITY);
      wheelmotor4->setPosition(INFINITY);
      wheelmotor1->setVelocity(0);
      wheelmotor2->setVelocity(0);
      wheelmotor3->setVelocity(0);
      wheelmotor4->setVelocity(0);
     }
    ...
    // data output
    mycout << i*0.032 << "\t\t" << performance_index << "\t\t" << stability_angle_deg << "\t\t" << contact_force1 << "\t\t"
     << contact_force2 << "\t\t" <<contact_force3 << "\t\t" <<contact_force4 << std::endl;
    if(i==3000){ delete robot; return 0;}
