// File:          Pose_Optimization_SA4.cpp
// Date:          2022.4.22
// Description:   优化方法：Simulated annealing algorithm; 3的基础上尝试解决由离地状态转为落地状态的异常
// Author:        Yin Siyuan
// Modifications: 
...
double last_beta1,last_beta2,last_beta3,last_beta4; // last position of pull motors
...
int main(int argc, char **argv) {
  ...
  while (robot->step(timeStep) != -1) {
    ...
    isContact = 1; 
    if(abs(contact_force1 <=0.1)){contact_flag1--;if(contact_flag1<0){contact_flag1=0;}}else{contact_flag1 = 1;}
    if(abs(contact_force2 <=0.1)){contact_flag2--;if(contact_flag2<0){contact_flag2=0;}}else{contact_flag2 = 1;}
    if(abs(contact_force3 <=0.1)){contact_flag3--;if(contact_flag3<0){contact_flag3=0;}}else{contact_flag3 = 1;}
    if(abs(contact_force4 <=0.1)){contact_flag4--;if(contact_flag4<0){contact_flag4=0;}}else{contact_flag4 = 1;}
    if(contact_flag1 == 0 || contact_flag2 == 0 || contact_flag3 == 0 || contact_flag4 == 0){isContact = 0;}
    
    if(isContact == 1){ contact_counter++;}else{contact_counter = 0;}
    ...
    else
    {
     ...
     if(isContact == 1)
     {
      ...
      {
       pullmotor1->setPosition(last_beta1 + 4.71);
       pullmotor2->setPosition(last_beta2);
       pullmotor3->setPosition(last_beta3);
       pullmotor4->setPosition(last_beta4);
      }     
    ...
    last_beta1 = beta1; last_beta2 = beta2; last_beta3 = beta3; last_beta4 = beta4;
    i++;
  };
