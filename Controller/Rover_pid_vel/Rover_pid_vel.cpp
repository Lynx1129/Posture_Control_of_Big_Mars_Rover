...
int main(int argc, char **argv) {
  ...
  double P_Pitch = 10; //pid control parameter
  double I_Pitch = 0.1;
  ...
  while (robot->step(timeStep) != -1) {
    ...
    else
    {
     ...
     double pullmotor_vel1 = std::min (std::max( vel_Pitch - vel_Roll,-0.2), 0.2);
     double pullmotor_vel2 = std::min (std::max( vel_Pitch + vel_Roll,-0.2), 0.2);
     double pullmotor_vel3 = std::min (std::max( - vel_Pitch - vel_Roll,-0.2), 0.2);
     double pullmotor_vel4 = std::min (std::max( - vel_Pitch + vel_Roll,-0.2), 0.2);
