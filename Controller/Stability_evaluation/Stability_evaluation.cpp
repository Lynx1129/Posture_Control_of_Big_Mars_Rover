// File:          Stability_evaluation.cpp
// Date:          2022.4.8
// Description:   evaluate the Stability of Rover (Using array)
// Author:        Yin Siyuan
// Modifications: 
...
// find a unit vector 
// double* unit_vector(double vector[3]) {
  // double length = sqrt(pow(vector[0],2)+pow(vector[1],2)+pow(vector[2],2));
  // double *unit_vector = new double[3]{vector[0]/length,vector[1]/length,vector[2]/length};
  // return unit_vector;
// }
...
int main(int argc, char **argv) {
  ...
  while (robot->step(timeStep) != -1) {
    ...
    // unit the contact point vector
    double *unitvector1 = unit_vector(contact_point1);  
    ...
    std::cout << "Pitch Roll Yaw is:  [\t"  << Roll << "\t" << Pitch << "\t" << Yaw <<"] (rad)" << std::endl;
    ...
    std::cout << "unit vector"<< unitvector1[0] << unitvector1[1] << unitvector1[2] <<std::endl; 
