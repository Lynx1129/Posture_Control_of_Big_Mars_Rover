// File:          gravity_change_test.cpp
// Date:          2022.4.14
// Description:   测试beta角度变化与重力方向变化的关系
// Author:        Yin Siyuan
// Modifications: 
...
const double k3 = 0; 
  ...
  const int timeStep = 16;  // get the time step of the current world.
  ...
  std::array<double,9> opt_para; //optimization equation initial condition parameter: 4 beta and 1 theta
...
std::array<double,9> stability_eva(std::array<double,9> parameter, std::array<double,3> gravity)
{
   ...
   double performance_index;
    ...
    mu1 = (round(100.0 * mu1))/100.0;
    mu2 = (round(100.0 * mu2))/100.0;
    mu3 = (round(100.0 * mu3))/100.0;
    mu4 = (round(100.0 * mu4))/100.0;
...
std::array<double,9> hill_climbing(std::array<double,9> parameter,std::array<double,3> gravity)
{ 
  // optimization step
  double step = 0.02;
  double iteration = 0;
  ...
  std::array<double,9> parameter_opt = parameter;
  std::array<double,9> parameter_result = parameter;
  
  while(iteration<2)
  {
    std::array<double,8> step_performance;
    ...
    parameter_opt.at(0) +=step; parameter_opt.at(0) = boundary(parameter_opt.at(0),0,0.4);
    parameter_opt.at(1) -=step; parameter_opt.at(1) = boundary(parameter_opt.at(1),-0.4,0);
    step_performance.at(0) = stability_eva(parameter_opt,gravity).at(8);
    parameter_opt.at(0) = previous_beta1; parameter_opt.at(1) = previous_beta2; 
    
    parameter_opt.at(0) -=step; parameter_opt.at(0) = boundary(parameter_opt.at(0),0,0.4);
    parameter_opt.at(1) +=step; parameter_opt.at(1) = boundary(parameter_opt.at(1),-0.4,0);
    step_performance.at(1) = stability_eva(parameter_opt,gravity).at(8);
    parameter_opt.at(0) = previous_beta1; parameter_opt.at(1) = previous_beta2; 
    
    parameter_opt.at(1) -=step; parameter_opt.at(1) = boundary(parameter_opt.at(1),-0.4,0);
    parameter_opt.at(3) +=step; parameter_opt.at(3) = boundary(parameter_opt.at(3),0,0.4);
    step_performance.at(2) = stability_eva(parameter_opt,gravity).at(8);
    parameter_opt.at(1) = previous_beta2; parameter_opt.at(3) = previous_beta4; 
    
    parameter_opt.at(1) +=step; parameter_opt.at(1) = boundary(parameter_opt.at(1),-0.4,0);
    parameter_opt.at(3) -=step; parameter_opt.at(3) = boundary(parameter_opt.at(3),0,0.4);
    step_performance.at(3) = stability_eva(parameter_opt,gravity).at(8);
    parameter_opt.at(1) = previous_beta2; parameter_opt.at(3) = previous_beta4; 
    
    parameter_opt.at(2) -=step; parameter_opt.at(2) = boundary(parameter_opt.at(2),-0.4,0);
    parameter_opt.at(3) +=step; parameter_opt.at(3) = boundary(parameter_opt.at(3),0,0.4);
    step_performance.at(4) = stability_eva(parameter_opt,gravity).at(8);
    parameter_opt.at(2) = previous_beta3; parameter_opt.at(3) = previous_beta4;
    
    parameter_opt.at(2) +=step; parameter_opt.at(2) = boundary(parameter_opt.at(2),-0.4,0);
    parameter_opt.at(3) -=step; parameter_opt.at(3) = boundary(parameter_opt.at(3),0,0.4);
    step_performance.at(5) = stability_eva(parameter_opt,gravity).at(8);
    parameter_opt.at(2) = previous_beta3; parameter_opt.at(3) = previous_beta4;
    
    parameter_opt.at(0) +=step; parameter_opt.at(0) = boundary(parameter_opt.at(0),0,0.4);
    parameter_opt.at(2) -=step; parameter_opt.at(2) = boundary(parameter_opt.at(2),-0.4,0);
    step_performance.at(6) = stability_eva(parameter_opt,gravity).at(8);
    parameter_opt.at(0) = previous_beta1; parameter_opt.at(2) = previous_beta3;
    
    parameter_opt.at(0) -=step; parameter_opt.at(0) = boundary(parameter_opt.at(0),0,0.4);
    parameter_opt.at(2) +=step; parameter_opt.at(2) = boundary(parameter_opt.at(2),-0.4,0);
    step_performance.at(7) = stability_eva(parameter_opt,gravity).at(8);
    parameter_opt.at(0) = previous_beta1; parameter_opt.at(2) = previous_beta3;
...
int main(int argc, char **argv) {
  ...
  while (robot->step(timeStep) != -1) {
    ...
    // beta optimization
    std::array<double,9> opt_result = hill_climbing(opt_para,gravity_normal);
    ...
    {
     pullmotor1->setPosition(4.92);
     pullmotor2->setPosition(-0.21);
     pullmotor3->setPosition(-0.2);
     pullmotor4->setPosition(0.2);
    }
